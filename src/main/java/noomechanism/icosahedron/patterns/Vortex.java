...
public class Vortex extends AnimPattern {
  ...
  public Vortex(LX lx) {
    super(lx);
    registerPhase("Ichi", 3.0f, 60.0f, "Start duration");
    registerPhase("Ni", 3.0f, 60.0f, "Explosion duration");
  ...
  public void runPhase(int phaseNum, double deltaMs) {
    ...
    for (LightBar lightBar : IcosahedronModel.getAllLightBars()) {
      if (phaseNum == 0) {
        ...
        blobPos1 = 0.5f + (time / curPhaseDuration)/ 2f - 0.01f;
        blobPos2 = 0.5f - (time / curPhaseDuration)/ 2f + 0.01f;
        LightBarRender1D.renderTriangle(colors, lightBar, blobPos1, slope.getValuef(), maxValue.getValuef(), LXColor.Blend.ADD);
        LightBarRender1D.renderTriangle(colors, lightBar, blobPos2, slope.getValuef(), maxValue.getValuef(), LXColor.Blend.ADD);
      }
    }
